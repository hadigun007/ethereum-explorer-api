...
// 3 2
    ...
    function addOwner(address _owner)public ownerDoesntExist(_owner) notNull(_owner) validRequirement(owners.length + 1, required){
        isOwner[_owner] = true;
        owners.push(_owner);
    ...
    function replaceOwner(address owner, address newOwner)public ownerExists(owner) ownerDoesntExist(newOwner){
    ...
    function external_call(address destination, uint value, uint dataLength, bytes memory data) internal returns (bool) {
        ...
        assembly {
            ...
            result := call(
                sub(gas(), 34710),
                destination, value, d, dataLength, x, 0
            )
        }
        return result;
    }
