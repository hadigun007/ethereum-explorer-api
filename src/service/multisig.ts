...
export class MultisigService {
    ...
    async submitTransaction(multisig:MultisigModel) {
        ...
        const receipt = new ReceiptModel()
        const contract = new this.web3.eth.Contract(abi,multisig.getAddress())
        await contract.methods.submitTransaction(multisig.getDestination(), 0, this.web3.utils.fromAscii("test"))
        .send({
            from: this.signer.address,
            gas:"1000000"
        })
    ...
    async confirmTransaction(multisig:MultisigModel) {
        const receipt = new ReceiptModel()
        const contract = new this.web3.eth.Contract(abi,multisig.getAddress())
        const sender = this.web3.eth.accounts.privateKeyToAccount(multisig.getPrivateKey()).address
        console.log(sender);
        
        await contract.methods.confirmTransaction(multisig.getTransactionId())
        .send({
            from: sender,
            gas:"1000000"
        })
    ...
    async getTransactionIds(multisig:MultisigModel) {
        const contract = new this.web3.eth.Contract(abi,multisig.getAddress())
        return await contract.methods.getTransactionIds(multisig.getFrom(), multisig.getDestination(), multisig.getPending(), multisig.getExecuted()).call()
    }
    async getConfirmationCount(multisig:MultisigModel) {
        const contract = new this.web3.eth.Contract(abi,multisig.getAddress())
        return await contract.methods.getConfirmationCount(multisig.getTransactionId()).call()
    }
}